...
public class WebServicesClientHelper {
	...
	public static ServiceList partialServiceNameSearch(UDDIInquiryPortType inquiry, String token, String partialName) throws Exception {
		//FindBusiness fb = new FindBusiness();
		...
		org.uddi.api_v3.FindQualifiers fq = new org.uddi.api_v3.FindQualifiers();
		fq.getFindQualifier().add(UDDIConstants.APPROXIMATE_MATCH);
		//fq.getFindQualifiers().getFindQualifier().add(UDDIConstants.APPROXIMATE_MATCH);
		//fs.getFindQualifiers().add(UDDIConstants.APPROXIMATE_MATCH);
		fs.setFindQualifiers(fq);
		Name searchname = new Name();
		searchname.setValue(partialName);
		fs.getName().add(searchname);
		System.out.println(inquiry.findService(fs).getServiceInfos().getServiceInfo().size());
