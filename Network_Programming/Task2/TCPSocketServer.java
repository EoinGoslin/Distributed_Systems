...
public class TCPSocketServer {
   ...
   public void listen() {
      while (true) {
         try {
            ...
            // Read in the message
            String msg = in.readLine();//msg is now "10 5"
            ...
            //GET data from msg
            String [] numbers = msg.split(" "); //numbers will hold two slots, each with a number in it
            int num = Integer.parseInt(numbers[0]) * Integer.parseInt(numbers[1]); //Parse to ints for calculation and store the value
            String nMessage =  "The result of " + numbers[0] +" * " + numbers[1] + " is " + num; 
            sendMessageToClient(socket, nMessage); //send the full message as a parameter along with socket for streams 
   ...
   public void sendMessageToClient(Socket socket, String msg) {
      ...
      BufferedWriter bw = new BufferedWriter(osw); // wrap in Buffered reader so can use methods such as write() and flush()
   ...
   public static void main(String[] args) {
      ...
      if (args.length == 1) {
         ...
         // killed.
         server.listen(); //calls listen method which contains blocking methods accept() which waits (listens)  for incoming data
      } else {
